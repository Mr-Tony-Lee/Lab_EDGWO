...
def edgwo_g(fitness_function, tmax, N , Dim , LB , UB ):
    EDGWO_T = 0.9               #經過多久比例的時間後執行梯度下降改
    ...
    def gradient(fitness_function,position):
        epsilon = 10**(-6)
        grad = np.zeros(Dim)
        temp = np.zeros(Dim)
        for i in range(Dim):
        ...
        return grad
    def Gradient_descent(position):
        GD_tmax = tmax-int(tmax*EDGWO_T)
        learning_rate = INITIAL_LEARNING_RATE
        for t in range(GD_tmax):
            grad = gradient(fitness_function,position)
